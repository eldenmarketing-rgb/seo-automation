/**
 * SNAPSHOT — ex-`config/site-mode-registry.ts` avant A1 (2026-08-23).
 *
 * Ces configs par site (voix de marque, intents, contenu, cocon) ont été migrées
 * dans `site_profiles` par `scripts/seed-site-registry.ts`, seul module à encore
 * lire ce fichier. Les DÉFAUTS génériques par mode vivent désormais dans
 * `config/mode-defaults.ts`, en TypeScript. Conservé pour la traçabilité, à
 * supprimer en A2.
 */

import type { SiteModeConfig } from './site-modes.js';

export const legacyModeRegistry: Record<string, SiteModeConfig> = {
