...
// ─── Backward Compatibility (research.ts API) ───────────────
...
/**
 * Drop-in replacement for researchKeywords() in research.ts.
 * Used by /keywords bot command.
 */
export async function researchKeywords(
  ...
  const enriched = await enrichedKeywordSuggestions(topic, location, _department, 40);
  return enriched.map(k => ({
    keyword: k.keyword,
    type: k.type,
    source: k.source,
  }));
...
/**
 * Drop-in replacement for formatKeywordsForTelegram() in research.ts.
 */
...
/**
 * Drop-in replacement for suggestPages() in research.ts.
 */
export function suggestPages(
  ...
  if (serviceKws.length > 0) {
    pages.push({ title: `${topic} à ${location}`, targetKeywords: serviceKws.slice(0, 8).map(k => k.keyword), type: 'service' });
  }
  if (priceKws.length >= 2) {
    pages.push({ title: `Prix ${topic} ${location} — Tarifs et devis`, targetKeywords: priceKws.map(k => k.keyword), type: 'blog' });
  }
  if (urgentKws.length >= 2) {
    pages.push({ title: `${topic} urgent ${location} — Disponible 24h/7j`, targetKeywords: urgentKws.map(k => k.keyword), type: 'service' });
  }
  if (infoKws.length >= 3) {
    pages.push({ title: `FAQ ${topic} ${location} — Questions fréquentes`, targetKeywords: infoKws.map(k => k.keyword), type: 'faq' });
  }
  if (aviKws.length >= 2) {
    pages.push({ title: `Meilleur ${topic} ${location} — Avis et comparatif`, targetKeywords: aviKws.map(k => k.keyword), type: 'blog' });
  }
