...
export function registerDeployCommand(bot: Bot<BotContext>) {
  bot.command('deploy', async (ctx) => {
    ...
    if (!siteArg) {
      ...
      for (const key of Object.keys(sites)) {
        ...
        const hasHook = !!readEnvByName(hookEnv);
  ...
  bot.callbackQuery(/^deploy:(.+)$/, async (ctx) => {
    ...
    await ctx.editMessageText(`Déploiement lancé pour: <b>${sites[siteKey]?.name}</b>`, {
      parse_mode: 'HTML',
    });
...
async function doDeploy(ctx: BotContext, siteKey: string) {
  ...
  if (!readEnvByName(hookEnv)) {
    await ctx.reply(
      `⛔ Pas de deploy hook configuré pour <b>${site.name}</b>\n\nAjoute <code>${hookEnv}</code> dans .env`,
      { parse_mode: 'HTML' },
    );
    return;
  }
