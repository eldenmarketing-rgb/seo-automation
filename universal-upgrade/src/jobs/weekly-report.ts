...
import { notifyGeneration } from '../notifications/telegram.js';
...
export async function generateWeeklyReport(): Promise<string> {
  ...
  for (const [siteKey, site] of Object.entries(sites)) {
    try {
      ...
      const thisWeek = await fetchGscData(
        siteKey,
        thisWeekStart.toISOString().split('T')[0],
        now.toISOString().split('T')[0]
      );

      // Données semaine dernière
      const lastWeek = await fetchGscData(
        siteKey,
    ...
    try {
      ...
      for (const candidate of candidates) {
        ...
        try {
          ...
          await notifyGeneration(siteKey, msg);
